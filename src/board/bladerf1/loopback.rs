...
impl BladeRf1 {
    pub async fn set_loopback(&self, lb: BladerfLoopback) -> Result<()> {
        ...
        match lb {
            BladerfLoopback::Firmware => {
                 ...
                 * frequency changes.
                 */
                self.lms.set_loopback_mode(BladerfLoopback::Lna3).await?;
                self.usb_set_firmware_loopback(true).await?;
            }
            _ => {
                ...
                let fw_lb_enabled: bool = self.usb_get_firmware_loopback().await?;

                if fw_lb_enabled {
                    self.usb_set_firmware_loopback(false).await?;
                }
    ...
    pub async fn get_loopback(&self) -> Result<BladerfLoopback> {
        // CHECK_BOARD_STATE(STATE_INITIALIZED);

        let mut lb = BladerfLoopback::None;

        let fw_lb_enabled = self.usb_get_firmware_loopback().await?;
        if fw_lb_enabled {
