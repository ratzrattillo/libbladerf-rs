...
impl BladeRf1 {
    ...
    pub fn read_sync(&self, buffers: &mut [&mut [Complex32]], timeout_us: i64) -> Result<usize> {
        ...
        for (idx, sample) in buf
            ...
            .enumerate()
        {
            while buffer_selector < num_buffers {
                // Calculate the index where to put a sample in the selected buffer.
                let index = idx % buffer_size;
                // select the next input buffer, if the previous one is full
                if index == 0 && idx > 0 {
                    buffer_selector += 1;
                }
                buffers[buffer_selector][index] = sample;
            }
        }
