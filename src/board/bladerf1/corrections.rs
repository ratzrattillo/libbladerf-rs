...
impl BladeRf1 {
    ...
    pub async fn get_correction(&self, ch: u8, corr: BladeRf1Correction) -> Result<i16> {
        //CHECK_BOARD_STATE(STATE_INITIALIZED);

        match corr {
            BladeRf1Correction::Phase => self.interface.nios_get_iq_phase_correction(ch).await,
            BladeRf1Correction::Gain => {
                let value = self.interface.nios_get_iq_gain_correction(ch).await?;

                /* Undo the gain control offset */
                Ok(value - 4096)
            }
            BladeRf1Correction::DcoffI => self.lms.get_dc_offset_i(ch).await,
            BladeRf1Correction::DcoffQ => self.lms.get_dc_offset_q(ch).await,
            // _ => {
    ...
    pub async fn set_correction(&self, ch: u8, corr: BladeRf1Correction, value: i16) -> Result<()> {
        //CHECK_BOARD_STATE(STATE_INITIALIZED);

        match corr {
            BladeRf1Correction::Phase => {
                self.interface.nios_set_iq_phase_correction(ch, value).await
            }
            BladeRf1Correction::Gain => {
            ...
            BladeRf1Correction::DcoffI => self.lms.set_dc_offset_i(ch, value).await,
            BladeRf1Correction::DcoffQ => self.lms.set_dc_offset_q(ch, value).await,
            // _ => {
            //     Err(anyhow!("Invalid correction type: {corr}"));
            // }
        }
