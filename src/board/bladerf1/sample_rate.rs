...
impl BladeRf1 {
    pub async fn set_sample_rate(&self, channel: u8, rate: u32) -> Result<u32> {
        //CHECK_BOARD_STATE(STATE_INITIALIZED);
        self.si5338.set_sample_rate(channel, rate).await
    }

    pub async fn get_sample_rate(&self, channel: u8) -> Result<u32> {
        //CHECK_BOARD_STATE(STATE_INITIALIZED);
        self.si5338.get_sample_rate(channel).await
    }
    ...
    pub async fn set_rational_sample_rate(
        &self,
        channel: u8,
        rate: &mut BladerfRationalRate,
    ) -> Result<BladerfRationalRate> {
        //CHECK_BOARD_STATE(STATE_INITIALIZED);
        self.si5338.set_rational_sample_rate(channel, rate).await
    }

    pub async fn get_rational_sample_rate(&self, channel: u8) -> Result<BladerfRationalRate> {
        // CHECK_BOARD_STATE(STATE_INITIALIZED);
        self.si5338.get_rational_sample_rate(channel).await
    }
}
