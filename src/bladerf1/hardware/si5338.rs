...
impl<'a> Si5338<'a> {
    ...
    pub(crate) fn get_sample_rate(
        ...
        Op::new(async move {
            ...
            if actual.numerator() != 0 {
                log::debug!(
                    "Fractional sample rate truncated during integer sample rate retrieval"
                );
            }
    ...
    pub(crate) fn get_rational_sample_rate(
        ...
        Op::new(async move {
            ...
            ms.update_base();
            self.read_multisynth(&mut ms).await?;
    ...
    pub(crate) fn set_smb_freq(&mut self, rate: u32) -> impl MaybeFuture<Output = Result<u32>> {
        Op::new(async move {
            ...
            if act.numerator() != 0 {
                log::trace!(
                    "Non-integer SMB frequency set from integer frequency, truncating output."
                );
            }
            if act.integer() > u32::MAX as u64 {
    ...
    fn rational_multisynth(
        ...
        Op::new(async move {
            ...
            self.write_multisynth(&ms).await?;
            Ok(RationalRate::new(
                actual.integer(),
                actual.numerator(),
                actual.denominator(),
