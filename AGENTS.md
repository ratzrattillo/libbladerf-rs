...
`.github/workflows/release.yml` is `workflow_dispatch`-only. All checks run as parallel gate jobs that mirror the CI suite plus cross-compiles (aarch64, windows-gnu), the wasm check, and conventional-commits validation (`git-cliff --unreleased`); the test jobs also run `public_api` and clippy covers the tokio feature set. `bump_and_tag` runs only after all gates pass and does no checks itself — there is no pre-release hook (`release.toml` has none), so keep all checks in gate jobs. `github_release` then creates the GitHub release and `publish` runs `cargo publish` with `CARGO_REGISTRY_TOKEN`. A local `cargo release` has no automatic gate — run `bash scripts/check.sh` first.
